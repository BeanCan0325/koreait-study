 ...
 * 	- 제공되는 기본기능 : save(),findById(),findAll(),delete() 등
 */
...
//														entity 넣고, 데이터 타입
	 ...
	 * 	- SELECT count(*) > 0 FROM member WHERE usr_id = ?
	 * 
...
//		Query Method : 많이쓰는거 쿼리 메서드로 만들어놓은거 
	 ...
	 * @return true: 이미 해당 사용자가 email를 사용중
	 * 		   false: 존재하지 않음(사용가능)
	 */
	 ...
	 * 	- JPA  메서드 이름 규칙에 따라 자동으로 쿼리가 생성됨
	 * 		> findBy필드명(매개변수)
	 * 		> SELECT * FROM memeber WHERE user_id =?
	 * 
	 * 주의할 점
